...
# Inferential statistics (Chi-square test for goodness of fit)
def chi_square_test(observed, expected, title):
    chi2, p = stats.chisquare(f_obs=observed, f_exp=expected)[:2]
...
# Expected values for chi-square test (uniform distribution)
expected_values = np.array([total_respondents / len(importance_values)] * len(importance_values))

# T-test for comparing means (example usage)
def t_test(data1, data2, title):
    t_stat, p_val = stats.ttest_ind(data1, data2)
    print(f"{title} - T-test")
...
chi_square_test(importance_values, expected_values, "Importance of Learning a Foreign Language")
chi_square_test(willingness_values, expected_values, "Willingness to Increase Practical Classes")
chi_square_test(study_preferences_values, expected_values, "Study Preferences")
chi_square_test(reasons_values, expected_values, "Reasons for Learning a Foreign Language")

# Perform a t-test between two groups (example)
t_test(importance_values, willingness_values, "Comparison of Importance and Willingness")
